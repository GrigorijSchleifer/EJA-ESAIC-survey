...
```
```{r}
# Digital Index - Technical possible with internet
survey_dt_associations %>%
    dplyr::filter(tech_internet != "Different (please comment)") %>%
    select(c(tech_internet, digital_index)) %>%
    # chaning levels for better presentation of the contingency table
    mutate(digital_index = factor(digital_index,
                                   levels = c("High_GDP_PC",
                                              "Middle_GDP_PC",
                                              "Low_GDP_PC"))) %>%
    mutate(tech_internet = factor(tech_internet,
                                  levels = c("Yes",
                                             "No (please comment)"), 
                                  labels = c("Yes",
                                             "No"))) %>%
    ...
    as.data.frame.matrix() %>% 
    rownames_to_column(var = "Answer") %>% 
    add_column(Question = rep("tech_internet", 2))
```
...
# Figure 1: Dichte - Technical possible with internet
tech_internet_dichte <- survey_dt_associations %>% 
    dplyr::filter(tech_internet != "I don´t know") %>% 
    select(c(tech_internet, dichte)) %>% 
    table() %>% 
    chisq.test()

# Figure 1: Digital-Index - Technical possible with internet
tech_internet_digital_index <- survey_dt_associations %>% 
    dplyr::filter(tech_internet != "I don´t know") %>% 
    select(c(tech_internet, digital_index)) %>% 
    table() %>% 
    chisq.test()
...
# Figure 1: Dichte - Technical possible with telefone
tech_telefone_dichte <- survey_dt_associations %>% 
    dplyr::filter(tech_telefone != "Different (please comment)") %>% 
    select(c(tech_telefone, dichte)) %>% 
    table() %>% 
    chisq.test()
# Figure 1: Digital-Index - Technical possible with telefone
tech_telefone_digital_index <- survey_dt_associations %>% 
    dplyr::filter(tech_telefone != "Different (please comment)") %>% 
...
# Chi-Square stats: DIGITAL INDEX - Technical possible with INTERNET
Digital_index_tech_internet <- survey_dt_associations %>%
    dplyr::filter(tech_internet != "Different (please comment)") %>%
