...
# Associations
```{r GDP_PC: Stacked bar horizontal: Internet possible}
# link to statistics page
# http://www.sthda.com/english/wiki/chi-square-test-of-independence-in-r

# Table: GDP - Technical possible with internet
...
# horizontal stacked bar plot for internet possible stritified with GDP_PC
plot_tech_internet <- survey_dt_associations %>%
    dplyr::filter(tech_internet != "Different (please comment)") %>%
    select(c(tech_internet, GDP_PER_CAPITA)) %>%
    # chaning levels for better presentation of the contingency table
    ...
    gather(key = "Index", value = "value", -Answer, -Question) %>% 
    ggplot(aes(x = value, y = Answer, fill=Index)) +
    geom_bar(position = "fill", stat='identity')
    
corrplot(
    GDP_tech_internet$residuals,
    is.corr = FALSE,
    tl.col = "black",
    tl.srt = 0,
    tl.cex = 1.4,
...
# trying out ggcorrplot package
prs_res_GDP_int <- as_data_frame(GDP_tech_internet$residual)
...
# trying geom_point approach
cols <- c("#d73027",
          "darkgrey",
          "#4575b4")

df %>% 
    ggplot(aes(x = GDP_PER_CAPITA, y = tech_internet, colour = n, size = n)) +
    geom_point() +
    scale_colour_gradientn(colours = cols) +
    theme(legend.position="bottom") +
    scale_size(range = c(1,20)) +
    guides(color=guide_legend(override.aes = list(size = c(20, 1, 20))))


# http://www.sthda.com/english/wiki/ggcorrplot-visualization-of-a-correlation-matrix-using-ggplot2
plot_pearsons_residuals_GDP_internet <- ggcorrplot(pearsons_residuals_GDP_internet, method = "circle")

ggballoonplot(prs_res_GDP_int_spread)

prs_res_GDP_int_spread %>% 
    ggplot() +
    geom_point()

ggarrange(plot_tech_internet,
          plot_pearsons_residuals_GDP_internet, 
          ncol = 2,
          nrow = 1)
...
```{r Correlation plot: Pearsons residuals}
corrplot(
    tech_internet_GDP$residuals,
    is.corr = FALSE,
    tl.col = "black",
    tl.srt = 0,
    tl.cex = 1.4,
