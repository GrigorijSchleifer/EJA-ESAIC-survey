...
```{r Figure 1 PIE: Technical question aranged by telefone and internet}
...
#           ncol = 2)
```
```{r Figure 2 Stacked barchart: Is an online/telephone informed consent for elective surgery in accordance with the legal requirements in your country? }
...
```{r Figure : GDP_PC Residuals: Internet/telefone possible}
...
```
```{r Figure : GDP_PC Residuals: Legal requirenments simple/complex/Redo}
...
survey_data %>% 
    select(Gender, `Your expert level`) %>% 
