...
# Associations
```{r Stacked bar horizontal: Internet possible}
# link to statistics page
# http://www.sthda.com/english/wiki/chi-square-test-of-independence-in-r

# Figure 1: GDP - Technical possible with internet
...
```
```{r Stacked bar horizontal: Telefone possible}
tech_internet_GDP <- survey_dt_associations %>%
...
# Figure 1: GDP - Technical possible with telefone
tech_telefone_GDP <- survey_dt_associations %>%
    dplyr::filter(tech_telefone != "Different (please comment)") %>% 
    select(c(tech_telefone, GDP_PER_CAPITA)) %>% 
    table() %>% 
    chisq.test()
