...
internet_simplelegend <- data.frame(internet_telefone = c(rep("Internet", 3), rep("Telefone", 3)),
                       answer = c("Depends", "Yes", "No", "Depends", "Yes", "No"),
                        when_simple = round(c(0.06647399, 0.23121387, 0.70231214, 0.06086957, 0.56666667, 0.37246377), 3) * 100)
...
internet_simplelegend <- internet_simplelegend %>% 
    ...
    facet_wrap(~factor(internet_telefone, levels = c("Internet", "Telefone")))
...
#           ncol = 2)
```
```{r Figure 2 PIE Is an online/telephone informed consent for elective surgery in accordance with the legal requirements in your country? }
...
consent_legal_stacked %>% 
    ...
    coord_flip()
```
```{r Figure 3: PIE When do you need to obtain Informed consent for elective surgery based on legal requirements?}
...
```{r GDP_PC Residuals: Internet/telefone possible}
...
```{r GDP_PC Residuals: Legal requirenments simple/complex/Redo}
...
# Legal requirenments for simple
corrplot(
...
# Legal requirenments for complex
corrplot(
...
```{r}
# Chi-Square stats: DIGITAL INDEX - Technical possible with TELEFONE
Digital_index_tech_telefone <- survey_dt_associations %>%
    dplyr::filter(tech_telefone != "Different (please comment)") %>%
    dplyr::filter(digital_index != "NO INDEX") %>%
    select(c(tech_telefone, digital_index)) %>%
    # chaning levels for better presentation of the contingency table
    mutate(digital_index = factor(
        digital_index,
        levels = c("High_Digital",
                   "Middle_Digital",
                   "Low_Digital")
    )) %>%
    mutate(
        digital_index = factor(
            digital_index,
            levels = c("High_Digital",
                       "Middle_Digital",
                       "Low_Digital"),
...
# Chi-Square stats: DICHTE - Technical possible with INTERNET
dichte_tech_internet <- survey_dt_associations %>%
    dplyr::filter(tech_internet != "Different (please comment)") %>%
    select(c(tech_internet, dichte)) %>%
    # chaning levels for better presentation of the contingency table
    mutate(dichte = factor(
        dichte,
        levels = c("Hohe_Dichte",
                   "Mittler_Dichte",
                   "Geringe_Dichte")
    )) %>%
    mutate(
        dichte = factor(
            ...
            levels = c("Hohe_Dichte",
                       "Mittler_Dichte",
                       "Geringe_Dichte"),
            labels = c("High Dicht ", "Middle Dichte","Low"))) %>%
    mutate(tech_internet = factor(
        tech_internet,
        levels = c("Yes",
                   "No (please comment)"),
...
# Chi-Square stats: DICHTE - Technical possible with TELEFONE
dichte_tech_telefone <- survey_dt_associations %>%
    dplyr::filter(tech_telefone != "Different (please comment)") %>%
    select(c(tech_telefone, dichte)) %>%
    # chaning levels for better presentation of the contingency table
    mutate(dichte = factor(
        dichte,
        levels = c("Hohe_Dichte",
...
# perscent contribution 
# contrib <- 100*GDP_tech_internet$residuals^2/GDP_tech_internet$statistic
...
df <- tibble(tech_internet = c("Yes", "No", "Yes", "No", "Yes", "No"),
             GDP_PER_CAPITA = c("High_GDP_PC", "High_GDP_PC", "Middle_GDP_PC", "Middle_GDP_PC", "Low_GDP_PC", "Low_GDP_PC"), 
             n = c(14.991455,-0.8520704,-0.9552603, 0.5429420, -11.1832737, 0.6725381))


ggarrange(plot_tech_internet,
          P1, 
          ncol = 2,
          nrow = 1)
...
#   Answer High_GDP_PC Middle_GDP_PC Low_GDP_PC      Question
# 1    Yes          89            49         19 tech_internet
# 2     No         222           181         83 tech_internet

```

```{r GDP_PC: Stacked bar horizontal: Telefone possible}
tech_internet_GDP <- survey_dt_associations %>%
    dplyr::filter(tech_telefone != "Different (please comment)") %>%
    select(c(tech_telefone, GDP_PER_CAPITA)) %>%
    # chaning levels for better presentation of the contingency table
    mutate(GDP_PER_CAPITA = factor(GDP_PER_CAPITA,
                                   levels = c("High_GDP_PC",
                                              "Middle_GDP_PC",
                                              "Low_GDP_PC"))) %>%
    mutate(tech_telefone = factor(tech_telefone,
                                  levels = c("Yes",
                                             "No (please comment)"), 
                                  labels = c("Yes",
                                             "No"))) %>%
    ...
    # add_column(Question = rep("tech_telefone", 2)) %>% 
    gather(key = "GDP_PC", value = "values", -Answer) %>% 
    ggplot(aes(x = GDP_PC, y = Answer))

#   Answer High_GDP_PC Middle_GDP_PC Low_GDP_PC      Question
# 1    Yes         175            56         24 tech_telefone
# 2     No         139           173         78 tech_telefone
```
